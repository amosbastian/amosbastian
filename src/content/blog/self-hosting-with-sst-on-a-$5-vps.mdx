...
I've been using [SST](https://sst.dev/) to deploy some of my projects to AWS for the past year. Some of these actually had more than 0 users. This + me being shit at coding + AWS = 💸.

Safe to say I was waking up in the middle of the night drenched in sweat worrying about my AWS bill (or not really since they gave me $1000s of credits, but still). Yes, this sounds like something a $5 VPS guy would say on 𝕏. Luckily for me, SST recently released an update that allows me to also become a $5 VPS guy.

<div class="flex items-center justify-center">
  <Tweet id="1818060092698210497" />
</div>

I'm going to show you how to use SST to self-host [Plunk](https://useplunk.com) with SST on Hetzner Cloud, all with a custom domain, https, and more. Even if you're not interested in self-hosting Plunk, I think this guide can still be useful to help you figure out how to self-host other stuff with SST.
...
I'm using Hetzner, but if you want to use Digital Ocean, or any of the other platforms out there, go ahead. For Hetzner all you'll need to do is sign up, create a project, head to the security section, and create an API token. Put this token in your `.env` file:
...
Note: SST currently only supports macOS, Linux, and WSL. Support for Windows should be coming soon.
...
Would you be surprised if I told you that you can do all of this with SST? I hope not, because you can. Run `sst add aws`, `sst add docker`, `sst add random` and add the following code to your `run` function:
...
const plunkSnsTopic = new aws.sns.Topic("SESTopic", {
  name: "ses-topic",
});
...
This will set up everything you need to self-host Plunk. Run `sst deploy` and you can view your self-hosted Plunk by visiting `http://your.server.ip:3000`.
...
Obviously you don't want to use your server's IP address to access Plunk over http. With SST we can also set up Cloudflare to point a custom domain to our server and Caddy (or any other reverse proxy) to handle https. Run `sst add cloudflare` and add the following code to your `run` function:
...
// const plunkDomain = $interpolate`http://${server.ipv4Address}`;
const plunkDomain = `plunk.${domain}`;
